onload = setTimeout(onWindowLoad, 300);

function onWindowLoad() {
	let loadingContainer = document.querySelector('.loading-container');
	let [loadingIcon1, loadingIcon2, loadingIcon3] = document.querySelectorAll('#first-iphone, #second-iphone, #third-iphone');

	loadingIcon1.classList.add('run-loading-animation-1');
	loadingIcon2.classList.add('run-loading-animation-2');
	loadingIcon3.classList.add('run-loading-animation-3');

	setTimeout(() => loadingContainer.remove(), 7900);

	let all = document.querySelectorAll(`.inner-camera-roll, .inner-camera, .square-container, .iphone-inner-photos, .video-container, .inner-weather,
  .inner-watch, .inner-clock, .inner-maps, .calc, .inner-reminders, .inner-iBooks, .inner-find-friends, .inner-video, .inner-video-container,
  .inner-find-iphone, .inner-telegram, .inner-telegram-settings, .opened-image, .opened-image-inner-favourites, .inner-extras-container,
  .inner-sololearn, .activity-feed-container-inner-sololearn`);

	all.forEach(block => block.style.display = 'block');
	all.forEach((block, idx) => setTimeout(() => block.style.display = 'block', idx * 10 || 50));
	setTimeout(() => all.forEach(block => block.style.display = 'none'), 6000);
	setTimeout(() => all[18].style.display = 'block', 7500);

	let sc = document.querySelector('.start-container');
	setTimeout(() => sc.style.display = 'none');
	setTimeout(() => sc.style.display = 'block', 3000);
	setTimeout(main, 500);
}
...
function main() {
	...
	setTimeout(() => iphone.style.transform = 'rotate(180deg)', 1500);
	setTimeout(() => iphone.style.transform = 'rotate(0deg)', 3000);

	let innerScreen = document.querySelector('.iphone-inner-screen');
	...
	let [imageHeartIcon, favouritesIconInnerCameraRoll] = document.querySelectorAll('#heart-icon, #favourites-icon-inner-camera-roll');
	...
	let openedImages = document.querySelectorAll('.opened-image');
...
❗️ Your passcode is ${passCode.split('').join(' ')} ❗️`);
	}, 8300);

	let passcodeInputIsAnimated = false;
	...
	let alarmInnerBottomContainer = document.querySelector('.alarm-inner-bottom-container');
	...
	let [tipsIcon, voiceIcon, contactsIcon, findFriendsIcon, findIphoneIcon] = document.querySelectorAll(`
  #tips-icon-inner-opened-extras, #voice-memos-icon-inner-opened-extras, #contacts-icon-inner-opened-extras,
  #find-friends-icon-inner-opened-extras, #find-iphone-icon-inner-opened-extras
  `);
  ...
  `;
	...
	circleInnerBedtimeContainer.onclick = () => {
		if (toggleInnerBedtimeContainer.style.background == 'rgb(102, 231, 58)') {
			...
			timeInnerMiddleContainer.style.color = '#ffffff';
			wakeTextInnerMiddleContainer.style.color = '#e9c600';
			wakeTimeInnerMiddleContainer.style.color = '#ffffff';
	...
	leftToggle.onclick = () => {
		if (extrasIsClicked || screenPos == 0) return;
		screenPos += 235;
		innerScreen.style.left = screenPos + 'px';

		if (screenPos == -235) {
			sixthPanel.style.left = -screenPos + 'px';
			simStatus.style.left = '239px';
			currentTimeHTML.style.left = '342.5px';
			batteryStatus.style.left = '446px';
		}

		if (screenPos == 0) {
			simStatus.style.left = '4px';
			currentTimeHTML.style.left = '107.5px';
			batteryStatus.style.left = '211px';
	...
	rightToggle.onclick = () => {
		if (extrasIsClicked || screenPos == -470) return;
		...
		if (screenPos == -470) {
			...
			currentTimeHTML.style.left = '577.5px';
			batteryStatus.style.left = '681px';
		}
	...
	calendarIcon.onclick = () => {
		...
		batteryStatus.style.background = 'black';
		batteryStatus.style.boxShadow = 'inset 1px 0px white, inset -1px 0px white, inset 0px 1px white, inset 0px -1px white';
		setTimeout(() => {
	...
	squareText.onclick = () => {
		if (innerCameraBottomContainerTopWrapper.style.left === '-43px') {
			return;
		}
		...
		setTimeout(() => {
			innerCameraBottomContainerTopWrapper.style.left = '-43px';
	...
	videoTextInnerCamera.onclick = () => {
		if (innerCameraBottomContainerTopWrapper.style.left == '37px') {
			return;
		}
		...
		setTimeout(() => {
			innerCameraBottomContainerTopWrapper.style.left = '37px';
	...
	let back = document.querySelector('.iphone-inner-screen-background');
	...
	extras.onclick = () => {
		...
		setTimeout(() => {
			innerScreen.style.filter = 'blur(4px)';
			back.style.filter = 'brightness(0.5) blur(4px)';
			innerExtras.style.opacity = '1';
		}, 0);
	...
	tipsIcon.onclick = () => {
		if (extrasIsClicked) {
			...
			innerScreen.style.filter = 'blur(0px)';
			back.style.filter = 'brightness(0.5) blur(1px)';
	...
	for (let i = 0; i < images.length; i++) {
		images[i].onclick = () => {
			...
			openedImages[i].style.display = 'block';
			openedImages[i].classList.add('run-appear-animation');
			innerImageBottomContainer.classList.add('run-bottom-container-appear-animation');

			if (i == 11) {
				imageHeartIcon.style.display = 'none';
				favouritesIconInnerCameraRoll.style.display = 'block';
			} else {
				imageHeartIcon.style.display = 'block';
				favouritesIconInnerCameraRoll.style.display = 'none';
			}

			setTimeout(() => {
				openedImages[i].classList.remove('run-appear-animation');
			...
			backArrowInnerImage.onclick = () => {
				...
				openedImages[i].classList.add('run-disappear-animation');
				innerImageBottomContainer.classList.add('run-bottom-container-disappear-animation');

				setTimeout(() => {
					openedImages[i].style.display = innerImageBottomContainer.style.display = 'none';
				}, 100);

				setTimeout(() => {
					openedImages[i].classList.remove('run-disappear-animation');
	...
	innerFavouritesImage.onclick = () => {
		...
		openedImageInnerFavourites.style.display = 'block';
	...
	homeButton.onclick = () => {
		...
		if (
			...
			innerScreen.classList.add('run-animation');

			setTimeout(() => {
				innerScreen.classList.remove('run-animation');
			}, 200);
		}
		...
		if (innerExtras.style.display == 'block') {
			...
			setTimeout(() => extrasIsClicked = false, 700);
		}
		...
		batteryStatus.style.display = 'inline';
		...
		batteryStatus.style.background = 'white';
		batteryStatus.style.boxShadow = 'inset 1px 0px black, inset -1px 0px black, inset 0px 1px black, inset 0px -1px black, inset 1px 1px black, inset -1px -1px black';

		sixthPanel.style.display = seventhPanel.style.display = eighthPanel.style.display = ninethPanel.style.display = '';
	}

	const correctHours = hours => hours >= 24 ? hours - 24 : hours < 0 ? 24 + hours : hours;
	...
	function createCalendar(elem, year, month) {
		...
		if (lastDayOfMonth == 31 && firstDayOfMonth == 0 || lastDayOfMonth == 31 && firstDayOfMonth == 6 || lastDayOfMonth == 30 && firstDayOfMonth == 0) {
			table += '</tr></table>';
		}
